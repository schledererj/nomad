...
func (w *nodeDrainWatcher) watch() {
	// note: Try to avoid logging in this function, which is a background
	// goroutine that is not waited on during a server shutdown. There is a race
	// condition between logging and the testing.T being complete which triggers
	// a panic, even if the test passes.
